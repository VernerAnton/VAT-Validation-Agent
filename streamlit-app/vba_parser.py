...
def entries_to_vba(entries: list[CountryEntry], header: str, footer: str) -> str:
    ...
    """
    # Update the Dim statement in the header to match actual count
    count = len(entries)
    header = re.sub(
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("Usage: python vba_parser.py <path_to_vba_file>")
        sys.exit(1)

    with open(sys.argv[1], 'r', encoding='utf-8') as f:
        text = f.read()

    entries, header, footer = parse_vba(text)
    print(f"Parsed {len(entries)} countries")

    # Round-trip test
    rebuilt = entries_to_vba(entries, header, footer)

    # Strip line numbers if present (the raw file has them)
    import re as _re
    original_stripped = _re.sub(r'^\s*\d+\t', '', text, flags=_re.MULTILINE)
    if rebuilt.strip() == original_stripped.strip():
        print("Round-trip: PASS — rebuilt file matches original")
    else:
        print("Round-trip: DIFF detected (may be line-number stripping)")
        # Show first difference
        for i, (a, b) in enumerate(zip(rebuilt.split('\n'), original_stripped.split('\n'))):
            if a != b:
                print(f"  Line {i+1}:")
                print(f"    rebuilt:  {a!r}")
